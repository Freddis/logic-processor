...
export class CanvasSquareGenerator {
  ...
  protected logger;
  elementListeners: Record<string, (el: ReactElement<CanvasElementProps>, state?: CanvasComponentState)=> void> = {};

  constructor(viewPort: RectCoords, invoker: string | undefined = undefined, debug = false) {
    this.viewPort = viewPort;
    this.debug = debug;
    this.logger = new Logger(CanvasSquareGenerator.name, invoker);
  ...
  addElement(el: ReactElement<CanvasElementProps>) {
    const xIndex = el.props.x / this.squareWidth;
    const yIndex = el.props.y / this.squareHeight;
    const id = xIndex + '_' + yIndex;
    const listener = this.elementListeners[id];
    if (!listener) {
  ...
  onElement(id: string, callback: (el: ReactElement<CanvasElementProps>, state?: CanvasComponentState)=> void) {
    this.elementListeners[id] = callback;
  }
